...
async function test1() {
    try {
        const aTiles = [{
            content: await loadPNGFromFile('t1.png')
        }, {
            content: await loadPNGFromFile('t2.png')
        }].map(t => ({content: pngToBase64(t.content)}));
        const output = await appendImages(aTiles, 0, 2);
        console.log('XXXXXXXXXXXXXXXXXXXX')
        const pngToSave = await loadPNGFromBase64(output.src);
        await savePNG('test-x.png', pngToSave);
    } catch (e) {
        console.error(e);
    }
}
