...
class Renderer {
    ...
    /**
     * liste des cellules balayées par les rayons lors du dernier rendu
     * @return {null|MarkerRegistry}
     */
    get visibleCells() {
    ...
    /**
     * liste des cellules balayées par le rayon CENTRAL lors du dernier rendu
     * @return {null|MarkerRegistry}
     */
    get visibleFrontCells() {
        return this._scanFrontCells;
    }

    /**
     * Cellules directement frappée par le rayon CENTRAL
     * @return {null}
     */
    get aimedCell() {
        return this._scanAimedCell;
    }
...
*/
    ...
    setBackground(oImage) {
        ...
        this._background = r === 1 ? oImage : CanvasHelper.resize(oImage, wNew, h);
    }
    ...
    */
    ...
    computeScreenSliceBuffer(scene) {
        ...
        let i;
        let zbuffer = [];
        let scanCells = new MarkerRegistry();     // registry of cells traversed by rays
        scanCells.mark(xCam8, yCam8);
        ...
        for (i = 0; i < xScreenSize; ++i) {
            const bMiddle = i === (xScreenSize >> 1);
            if (i >= xl && i <= xr) { // checks limits
                scene.resume.b = false;
                if (bMiddle) {
                    const mr = new MarkerRegistry();
                    this.castRay(scene, xCamera, yCamera, fBx, fBy, i, mr, zbuffer);
                    this._scanFrontCells = mr;
                    scanCells.merge(mr);
                    this._scanAimedCell = {
                        xCell: scene.xCell,
                        yCell: scene.yCell,
                    ...
                    this.castRay(scene, xCamera, yCamera, fBx, fBy, i, scanCells, zbuffer);
                }
            }
            fBx += dx;
            fBy += dy;
...
*/
    ...
    static renderScreenSlice(zbi, rc) {
        ...
        const bFxAlpha = (nFx & 12) !== 0;
        const bFxLighter = (nFx & 1) !== 0;
        if (bFxLighter) {
        ...
        let wSrc = aLine[3] | 0;
        let hSrc = aLine[4] | 0;
        const di_sy = aLine[2] * flagSourceFactor | 0;
        const di_dx = aLine[5] | 0;
        const di_dy = aLine[6] | 0;
        const di_dw = aLine[7] | 0;
        const di_dh = aLine[8] | 0;
    ...
    addLightSource(x, y, r0, r1, v) {
        ...
        oReactiveRCLS.events.on('changed', ({key, root}) => {
            if (key === 'v') {
                ...
                source.metrics[key] = oRayCasterLightSource[key] * r | 0;
            }
        });
...
//     |_|                                                            |___/
    ...
    renderSprite(scene, oSprite) {
        ...
        if (Math.abs(fAlpha) <= (fov * 1.5)) {
            ...
            const dw = wspr * factor / scale;               // projection of width
...
//  \__\___/_/\_\\__|\__,_|_|  \___|  \__,_|_| |_|_|_| |_| |_|\__,_|\__|_|\___/|_| |_|
    ...
    /**
     * Returns the number of bytes consumed by all tilesets
     * @return {number}
     */
    getGraphicsConsumption() {
        return this._tilesets.reduce((prev, curr) => {
            /**
             * @var curr {ShadedTileSet}
             */
            return prev + curr.getConsumedBytes();
        }, 0);
    }
}
