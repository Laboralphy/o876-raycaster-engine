  ...
  >
    ...
    <form>
      ...
      <fieldset>
        ...
        <div>
          <label>Camera Thinker:
            <input v-model="value.cameraThinker" type="text"/>
          </label>
          <div class="hint">Reference of the camera thinker (the which control its behavior).</div>
        </div>
      </fieldset>
      <fieldset>
        ...
        <div>
          ...
          <div class="hint">If checked, the second story wall textures will be stretched, and will appear twice
            taller.
          </div>
        </div>
      </fieldset>
      <fieldset>
        ...
        <div>
          ...
          <div class="hint">If checked, each time you save the level, it will also be published to the local game
            project.
          </div>
        </div>
      </fieldset>
      <br/>
      <div>
        <MyButton :disabled="indicator.length > 0" @click="applyClicked">Apply changes</MyButton>
        {{ indicator }}
      </div>
      <h3>Warning</h3>
      ...
      <p>
        Hint : Setting tile width (and height) should be the very first operation you do just after starting a new level
        from scratch.
      </p>
    </form>
  </Window>
...
export default {
  ...
  methods: {
    ...levelMapAction({
      ...
      setCameraThinker: LEVEL_ACTIONS.SET_CAMERA_THINKER
    }),
    ...
    applyClicked: async function () {
      ...
      await this.setCameraThinker({value: this.value.cameraThinker});
